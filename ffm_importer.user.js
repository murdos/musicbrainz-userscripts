...
(function () {
    ...
    function readPageCache() {
      try {
        const parsed = JSON.parse(window.localStorage.getItem(pageCacheKey()) ?? 'null');
    ...
    function collectServiceAnchors() {
    ...
    function waitForServiceAnchors() {
    ...
    async function resolveServiceLinks(anchors, cache) {
      const resolved = await Promise.all(anchors.map(async anchor => {
    ...
    `;
      mountPanel(root);
      const serverSelect = root.querySelector('.ffm-mb-server');
      ...
      return {
        root,
        status: root.querySelector('.ffm-mb-status'),
        release: root.querySelector('.ffm-mb-release'),
        server: serverSelect,
        harmonyButton: root.querySelector('.ffm-mb-harmony'),
        missingLinksButton: root.querySelector('.ffm-mb-missing'),
        missingLinksLabel: root.querySelector('.ffm-mb-missing-label')
      };
    }
    function mountPanel(root) {
      const musicServices = document.querySelector('.music-services-section');
      if (musicServices?.parentElement) musicServices.parentElement.insertBefore(root, musicServices);else document.body.appendChild(root);
    }
    function keepPanelMounted(panel, onRemount) {
    ...
    function markExistingLinks(anchors, links, matchedUrls) {
    ...
    function submitMissingLinks(server, releaseId, links) {
      ...
      form.hidden = true;
      const parameters = [['edit_note', `Added URL relationships from ${window.location.href.replace(/[?#].*$/, '')}\n\nUsing '''${GM_info.script.name}''' ${GM_info.script.version} from https://github.com/murdos/musicbrainz-userscripts`], ['redirect_uri', `${server}/release/${releaseId}`]];
