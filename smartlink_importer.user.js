...
// @updateURL    https://raw.githubusercontent.com/murdos/musicbrainz-userscripts/dist/smartlink_importer.user.js
...
(function () {
    ...
    function nextCacheKey(counters, service) {
    ...
    function extractAlbumLinkPageData(payload) {
    ...
    function cleanAlbumLinkPageUrl(currentUrl, canonicalUrl) {
      let url;
      try {
    ...
    function actionFromAriaLabel(ariaLabel) {
    ...
    function serviceLabelFromAriaLabel(ariaLabel) {
      return ariaLabel.match(/\s+on\s+(.+)$/i)?.[1]?.trim() ?? '';
    }
    function collectAlbumLinkServiceElements() {
    ...
    function collectBandLinkServiceElements() {
      const counters = new Map();
      const elements = [];
    ...
    function serviceLabel(serviceName) {
      return serviceName.split(/[-_\s]+/).filter(Boolean).map(word => `${word.charAt(0).toUpperCase()}${word.slice(1)}`).join(' ');
    }

    /** Read active provider destinations from Fanlink’s `window.preloadLink` payload. */
    function extractFanlinkServiceData(payload) {
    ...
    function extractFanlinkServiceDataFromScript(source) {
      ...
      try {
        return extractFanlinkServiceData(JSON.parse(serializedPayload));
      } catch {
        return [];
      }
    }
    ...
    function serviceFromElement(element) {
      const imageUrl = element.querySelector('.link-option-row-img')?.src;
      if (!imageUrl) return '';
      try {
        return normalizeServiceName(new URL(imageUrl).pathname.split('/').pop()?.replace(/\.[^.]+$/, '') ?? '');
      } catch {
        return '';
      }
    }
    function collectFanlinkServiceElements() {
      ...
      const counters = new Map();
      const elements = [];
      for (const element of document.querySelectorAll('.link-options a.link-option-row')) {
    ...
    function collectFfmServiceElements() {
      const counters = new Map();
      const elements = [];
    ...
    function smartLinkSiteForHostname(hostname) {
      ...
      for (const [site, domains] of Object.entries(DOMAINS_BY_SITE)) {
