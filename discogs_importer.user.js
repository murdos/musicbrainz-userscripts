...
function parseRelease(xmldoc) {
    ...
    var releasedate = getXPathVal(xmldoc, "//release/released", true).split('-');

    release.year = parseInt(releasedate[0], 10);

    release.month = parseInt(releasedate[1], 10);

    release.day = parseInt(releasedate[2], 10);
...
getXPathVal = function (xmldoc, xpathExpression, wantSingleNode, rootNode) {
    ...
    if (wantSingleNode) {
        ...
        return xmldoc.evaluate(xpathExpression, xmldoc, rootNode, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue.textContent;
