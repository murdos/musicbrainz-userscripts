...
function buildFormParameters(release) {    
    ...
    for (var i=0; i < release.artist_credits.length; i++) {
        var ac = release.artist_credits[i];
    	appendParameter(parameters, 'artist_credit.names.'+i+'.name', ac.anv);
        appendParameter(parameters, 'artist_credit.names.'+i+'.artist.name', ac.name);

    	appendParameter(parameters, 'artist_credit.names.'+i+'.join_phrase', ac.join);
    }
