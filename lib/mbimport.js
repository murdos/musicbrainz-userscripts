...
`;
    ...
    function fnExactSearchUrlFor(type, what, limit) {
        type = type.replace('-', '_');
        ...
        let params = [`query=${encodeURIComponent(query)}`, `type=${type}`, `limit=${limit}`, 'method=advanced'];
        return `https://musicbrainz.org/search?${params.join('&')}`;
    }
    ...
    function fnBuildFormParameters(release, edit_note) {
        ...
        if (!release.type && release.title) {
            ...
            let complete_duration = allTracksHaveDuration ? total_duration : NaN;
            release.type = fnGuessReleaseType(release.title, total_tracks, complete_duration, track_titles);
        }
    ...
    function normalizeTrackTitle(title) {
        ...
        normalized = normalized.replace(/\s*[([{]([^\])}]*?)[\])}]/g, function (match, contents) {
            return VERSION_MARKER.test(contents) ? '' : match;
        });
        normalized = normalized.replace(/\s*[-–—:]\s*([^\n]*)$/, function (match, suffix) {
            return VERSION_MARKER.test(suffix) ? '' : match;
        });
        return normalized
            .replace(/[^\p{L}\p{N}]+/gu, ' ')
            .trim()
    ...
    function isMultiTrackSingle(num_tracks, track_titles) {
        if (
            !Array.isArray(track_titles) ||
            num_tracks < 2 ||
            track_titles.length !== num_tracks ||
            track_titles.some(function (title) {
                return typeof title !== 'string';
            })
        ) {
            return false;
        }
        const normalized_titles = track_titles.map(normalizeTrackTitle);
        return normalized_titles.every(function (title) {
            return title.length > 0 && title === normalized_titles[0];
        });
    }
     ...
     */
    function fnGuessReleaseType(title, num_tracks, duration_ms, track_titles = []) {
        if (!Number.isInteger(num_tracks) || num_tracks < 1) return '';

        const release_title = typeof title === 'string' ? title : '';
        const has_single = /\bsingle\b/i.test(release_title);
        const has_EP = /\bE\.?P\b\.?/i.test(release_title);
        const has_duration = Number.isFinite(duration_ms) && duration_ms > 0;
        const duration_minutes = has_duration ? duration_ms / 60_000 : NaN;

        if (has_EP) return 'EP';
        if (has_single && num_tracks <= 8 && (!has_duration || duration_minutes <= 50)) return 'single';
        if (isMultiTrackSingle(num_tracks, track_titles)) return 'single';

        if (!has_duration) {
            if (num_tracks === 1) return 'single';
            if (num_tracks >= 3 && num_tracks <= 6) return 'EP';
            if (num_tracks >= 7) return 'album';
            return '';
        }

        if (num_tracks >= 7) return 'album';
        if (duration_minutes > 30) return 'album';
        if (duration_minutes < 1) return '';
        if (duration_minutes <= 7) return 'single';
        if (num_tracks >= 2) return 'EP';
        return '';
    }
    ...
    return {
        ...
        exactSearchUrlFor: fnExactSearchUrlFor,
        URL_TYPES: url_types,
