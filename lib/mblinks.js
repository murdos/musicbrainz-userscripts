...
const MBLinks = function (user_cache_key, version, expiration) {
        ...
        const doRequest = () => {
            ...
            requestJSON(url)
                ...
                .catch(error => {
                    ...
                    if (is5xx) {
                        const retryDelayMs = Math.max(error.retryDelayMs || 0, Math.min(30_000, 1000 * 2 ** (attempt - 1)));
                        if (Date.now() + retryDelayMs <= retryDeadline) {
                            setTimeout(() => {
                                this.scheduleRequest(doRequest);
                            }, retryDelayMs);
                        } else if (typeof alwaysCallback === 'function') alwaysCallback(false);
    ...
    this.runNextRequest = function () {
        if (requestTimer || pendingRequests.length === 0) return;
