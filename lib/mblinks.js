...
//  mblinks.searchAndDisplayMbLink(artist_link, 'artist', function (link) { document.querySelector('div#there').insertAdjacentHTML('beforebegin', link); } );
//
//  var album_link = 'http://' + window.location.href.match( /^https?:\/\/(.*\/album\/.+)$/i)[1];
//  mblinks.searchAndDisplayMbLink(album_link, 'release', function (link) { document.querySelector('div#there').insertAdjacentHTML('afterend', link); } );
// }
...
const MBLinks = function (user_cache_key, version, expiration) {
     ...
     */
    function processUrlMatch({ mblinks, batch, resource, relations, foundQueries }) {
    ...
    const recordRequestFailure = serverDelayMs => {
        consecutiveRequestFailures += 1;
        const retryDelayMs = Math.max(serverDelayMs, Math.min(30_000, 1000 * 2 ** (consecutiveRequestFailures - 1)));
    ...
    function requestJSON(url) {
        const modernGM = Reflect.get(globalThis, 'GM');
        const gmRequest = modernGM?.xmlHttpRequest ?? Reflect.get(globalThis, 'GM_xmlhttpRequest');
        if (!gmRequest) {
    ...
    this.getJSONWithRetry = function (url, successCallback, alwaysCallback) {
        ...
        const retry = (serverDelayMs = 0) => {
            const retryDelayMs = recordRequestFailure(serverDelayMs);
        ...
        const doRequest = async () => {
            ...
            if (!response.ok) {
                if (response.status >= 500 && response.status < 600) {
                    ...
                    try {
                        successCallback({});
                        if (typeof alwaysCallback === 'function') alwaysCallback(true);
                    } catch {
                        if (typeof alwaysCallback === 'function') alwaysCallback(false);
                    }
                } else {
                    consecutiveRequestFailures = 0;
                    if (typeof alwaysCallback === 'function') alwaysCallback(false);
    ...
    this.scheduleRequest = function (request, priority = false) {
        if (priority) pendingRequests.unshift(request);
        else pendingRequests.push(request);
        this.runNextRequest();
    };

    this.pauseRequests = function (delayMs) {
        rateLimitResetAt = Math.max(rateLimitResetAt, Date.now() + delayMs);
    };

    this.runNextRequest = function () {
        if (requestTimer || requestInFlight || pendingRequests.length === 0) return;

        const now = Date.now();
        const runAt = Math.max(now, nextRequestAt, rateLimitResetAt);
    ...
    this.searchAndDisplayMbLink = function (url, mb_type, insert_func, key, onComplete) {
        ...
        if (this.is_cached(key)) {
            ...
            if (typeof onComplete === 'function') {
                onComplete({ found: true, status: 'success' });
            }
        } else {
            ...
            mblinks.ajax_requests.push(
                ...
                // handler
                function () {
                    let ctx = this; // context from handler.bind()
                    let mbl = ctx.mblinks;
                    let found = false;
                    mbl.getJSONWithRetry(
                        ...
                        function (succeeded) {
                            const result = { found: succeeded && found, status: succeeded ? 'success' : 'error' };
                            ctx.completionHandlers.forEach(fn => fn(result));
                        },
                    );
                },
    ...
    this.searchAndDisplayMbLinks = function (urls_data) {
        ...
        for (let i = 0; i < uncached_urls.length; i += BATCH_SIZE) {
            ...
            if (query in mblinks.ajax_requests) {
                handlers = mblinks.ajax_requests[query].context.handlers;
                failureHandlers = mblinks.ajax_requests[query].context.failureHandlers;
            }
    ...
    this.enqueueRegexSearchPage = function (batch, regex, offset, lookup) {
        lookup.pending += 1;
        const mblinks = this;
    ...
    this.finishRegexOperation = function (lookup) {
        lookup.pending -= 1;
